...
def run_server():
    ...
    if topic_check.topic_exists("com.udacity.tables.turnstile_summary") is False:
        logger.fatal(
            "Ensure that the KSQL Command has run successfully before running the web server!"
        )
        exit(1)
    if topic_check.topic_exists("com.udacity.faust.streams.stations") is False:
        logger.fatal(
    ...
    consumers = [
        KafkaConsumer(
            "com.udacity.streams.weather-status-items",
        ...
        KafkaConsumer(
            "^com.udacity.streams.stations.[^.]+.arrivals",
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
